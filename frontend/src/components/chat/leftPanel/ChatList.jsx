...
function ChatList({ selectedChat, setSelectedChat }){
  ...
  useEffect(() => {
    ...
    const eventSource = new ReconnectingEventSource(
        `http://127.0.0.1:8000/api/v1/chat/notifications/${user.user_id}/${accessToken}/events/`
    )
    eventSource.onopen = () => {
  ...
  function getChatDescription(chat){
    for (let active_user in chat.status){
      if (
          chat.status.hasOwnProperty(active_user) &&
          active_user.toString() !== user.user_id.toString()
      ){
  ...
  const getLastMessageStatus = (chat) => {
    if(chat.last_message_info) {
      if (chat.last_message_info.sender === user.user_id) {
        if (chat.last_message_info.is_read){
          return <BiCheckDouble className="purple"/>
        }
        else {
          return <BiCheck className="purple"/>
        }
      }
    }
  }
